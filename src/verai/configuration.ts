...
export const IndexConfigurationAnnotation = Annotation.Root({
  ...
  /**
   * Name of the collection in the vector database (e.g., Qdrant).
   */
...
export function ensureIndexConfiguration(
  ...
  return {
    ...
    embeddingModel:
      configurable.embeddingModel || 'openai/text-embedding-3-large',
    retrieverProvider: configurable.retrieverProvider || 'qdrant',
    collectionName: configurable.collectionName || 'default',
...
export const ConfigurationAnnotation = Annotation.Root({
  ...
  /**
   * Company name related to the ticket.
   */
  ...
  /**
   * Client identifier (external system).
   */
...
export function ensureConfiguration(
  ...
  return {
    ...
    companyName: configurable.companyName || 'default',
    ticketId: configurable.ticketId || 'default',
    clientId: configurable.clientId || 'default',
  };
}
