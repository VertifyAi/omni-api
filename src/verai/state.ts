...
export const StateAnnotation = Annotation.Root({
  ...
  /**
   * Stores the responsible department information.
   * @type {Object}
   * @description Stores information about the department responsible for handling the user's request
   */
  responsibleDepartment: Annotation<{
    name: string;
    contact: string;
    description: string;
  } | null>({
    value: (_, newValue) => newValue,
    default: () => null,
  }),

  /**
   * Stores the user information collected during the conversation.
   * @type {Object}
   */
  userInfo: Annotation<{
    name: string | null;
  ...
  /**
   * Controls the current step of the conversation flow.
   * @type {string}
   */
  conversationStep: Annotation<string>({
    value: (existing, newValue) => {
      // Se já existe um valor e não foi fornecido um novo, preserva o existente
      if (existing && (!newValue || newValue === 'main_conversation')) {
        return existing;
      }
      // Caso contrário, usa o novo valor ou o padrão
      return newValue || 'initialization';
    },
